...
`loop-ad_event_sdk`로 `promotion_impression`, `promotion_click` 이벤트를 보냅니다.
...
function trackAdImpression(decision: AdvertisementFilledDecision): void {
  eventSdk.track("promotion_impression", decision.tracking);
}

function trackAdClick(decision: AdvertisementFilledDecision): void {
  eventSdk.track("promotion_click", decision.tracking);
}
...
`BANNER_ASSIGNMENT_NOT_FOUND` 404를 반환하면 SDK는 `empty` decision으로 처리합니다.
...
| option | 필수 | 기본값 | 설명 |
|---|---:|---|---|
| `placementId` | yes | 없음 | Dashboard에서 정의한 광고 지면 id. `placementKey`도 호환 alias로 받을 수 있습니다. |
| `targetId` | yes | 없음 | 광고 DOM을 넣을 element id |
| `context` | no | 없음 | 이전 SDK 호환용 필드입니다. 현재 banner resolve API에는 전송하지 않습니다. |
| `onImpression` | no | `null` | 광고가 50% 이상 보이면 1회 호출되는 callback |
| `onClick` | no | `null` | 광고 클릭 시 target URL 이동 전에 호출되는 callback |

`decision.tracking`에는 지표 집계에 필요한
`campaign_id`, `promotion_id`, `promotion_run_id`, `ad_experiment_id`, `segment_id`,
`content_id`, `content_option_id`, `promotion_channel`, `placement_id`, `target_url`이
포함됩니다.
...
- `status`가 `filled`이면 `a`, `strong`, `span`만 만들어 target에 넣습니다.
...
- 클릭 시 `onClick(decision)`을 먼저 호출하고, `targetUrl`이 있으면
  `window.location.assign(targetUrl)`로 이동합니다.
